...
cGoldDisplay::cGoldDisplay()
    : cStatusText()
{
    Set_Jewels(pLevel_Player->m_goldpieces);
}
...
void cGoldDisplay::Set_Jewels(int gold)
{
    while (gold >= 100) {
        gold -= 100;
        ...
        gp_hud->Add_Points(0, pLevel_Player->m_pos_x + pLevel_Player->m_image->m_w/3, pLevel_Player->m_pos_y + 5, "1UP", lightred);
...
void cGoldDisplay::Add_Jewels(int gold)
{
    Set_Jewels(pLevel_Player->m_goldpieces + gold);
}
